...
{
    public partial class csNavBar : UserControl
    {
        #region Variables
        ...
        private void csNavBar_Resize(object sender, EventArgs e)
        {
        ...
        public void Initialize(List<REMAS.NavBarItem> navbar, REMAS.Controller.NavBar.Themes.ITheme theme)
        ...
        public void AddItem(int parentID, string text, int itemID, int leftChild = 10)
        {
            foreach (var nbi in _navBarItems)
            {
                ...
                {
                    if (nbi.Childs == null)
                        nbi.Childs = new List<REMAS.NavBarItem>();

                    REMAS.NavBarItem newItem = new REMAS.NavBarItem { ID = itemID, ParentID = parentID, Text = text };
        ...
        private REMAS.NavBarItem _JRIT_item;
        /// <summary>
        ...
        /// </summary>
        public void SetTheme(REMAS.Controller.NavBar.Themes.ITheme theme)
        {
            _theme = theme;
            DrawControlItems();
        }
        ...
        /// </summary>
        /// <param name="item"></param>
        private void DrawChilds(REMAS.NavBarItem item)
        {
        ...
        private void ResetAll(REMAS.NavBarItem item)
        {
            foreach (var c in item.Childs)
            {
                c.Selected = false;
        ...
        private void ResetAllSelect(REMAS.NavBarItem item, int itemID)
        {
            foreach (var c in item.Childs)
            {
                c.Selected = false;
        ...
        private void GetAllParentsChild(REMAS.NavBarItem clickedItem, REMAS.NavBarItem item, int? parentID)
        {
            foreach (var c in item.Childs)
            {
        ...
        private void GetAllParents(REMAS.NavBarItem clickedItem)
        {
            foreach (var nbi in _navBarItems)
            {
        ...
        private void AutoExpand(REMAS.NavBarItem clickedItem, REMAS.NavBarItem item)
        {
        ...
        /// </summary>
        /// <param name="item"></param>
        private void OnItemClick(REMAS.NavBarItem item)
        {
        ...
        private void AddChild(REMAS.NavBarItem item, int parentID, string text, int itemID, int leftChild = 10)
        {
            foreach (var c in item.Childs)
            {
                if (c.ID == parentID)
                {
                    if (c.Childs == null)
                        c.Childs = new List<REMAS.NavBarItem>();

                    REMAS.NavBarItem newItem = new REMAS.NavBarItem { ID = itemID, ParentID = parentID, Text = text, LeftChild = leftChild };
        ...
        private void EnableItemChild(REMAS.NavBarItem item, bool enable, int itemID)
        {
            foreach (var c in item.Childs)
            {
                if (c.ID == itemID)
        ...
        private void VisibilityItemChild(REMAS.NavBarItem item, bool visible, int itemID)
        {
            foreach (var c in item.Childs)
            {
                if (c.ID == itemID)
        ...
        private class NavBarItemPanel : Panel
        {
            ...
            public REMAS.NavBarItem _navItem;
            /// <summary>
            /// Event we trigger when click on the panel control.
            /// </summary>
            /// <param name="item"></param>
            public event ItemClickEventHandler ItemClick;
            public delegate void ItemClickEventHandler(REMAS.NavBarItem item);

            #endregion

            #region "Initialization"

            public void Initialize(REMAS.NavBarItem navItem, int width, REMAS.Controller.NavBar.Themes.ITheme theme, int depth)
            {
                _navItem = navItem;
                _theme = theme;
                _depth = depth;
