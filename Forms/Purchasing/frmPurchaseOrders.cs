...
namespace REMAS
{
    public partial class frmPurchaseOrders : Form
    {
        ...
        private string GenerateNextReferenceCode()
        {
            ...
            string query = "SELECT MAX(ref_purchaseOrder) FROM HTA_PurchaseOrder WHERE ref_purchaseOrder LIKE @Prefix + '%'";
        ...
        {
            var companies = new List<CompanyPO>();
        ...
        private void btnRequest_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var selectedContact = (ContactPO)cmbContact.SelectedItem;
                string refContact = selectedContact.RefContact;
                string contactName = selectedContact.FirstName;

                if (conn.State == ConnectionState.Closed) conn.Open();
                sql = "HTA_sp_InsertPurchaseOrder";
                using (cmd = new SqlCommand(sql, conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    cmd.Parameters.AddWithValue("@ref_purchaseOrder", refCode);
                    cmd.Parameters.AddWithValue("@compName", ((CompanyPO)cmbCompany.SelectedItem).Name);
                    cmd.Parameters.AddWithValue("@refSupplier", ((SupplyPO)cmbSupplier.SelectedItem).RefSupplier);
                    cmd.Parameters.AddWithValue("@supplierName", ((SupplyPO)cmbSupplier.SelectedItem).Name);
        ...
        private void btnValidated_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (conn.State == ConnectionState.Closed) conn.Open();
                sql = "UPDATE HTA_PurchaseOrder SET status = @status WHERE ref_purchaseOrder = @ref_purchaseOrder";
                cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@status", status);
                cmd.Parameters.AddWithValue("@ref_purchaseOrder", currentRefCode);
        ...
        private bool IsRecordFinished(string refCode)
        {
            try
            {
                ...
                string query = "SELECT status FROM HTA_PurchaseOrder WHERE ref_purchaseOrder = @refCode";
        ...
        private void cmbSupplier_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var selectedSupplier = (SupplyPO)cmbSupplier.SelectedItem;
        ...
        {
            var suppliers = new List<SupplyPO>();
                ...
                using (cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            cmbContact.Items.Add(new ContactPO { RefContact = refContact, FirstName = firstName });
                        }
